...
export default async function handler(req, res) {
  ...
  try {
    const { testEmail, email: testEmailAddress } = req.body;

    // Get subscribers from Supabase
    let subscribers = [];
    
    if (testEmail && testEmailAddress) {
      // Send test email to specific address
      subscribers = [{ email: testEmailAddress }];
    } else {
    ...
    console.log(`VIP Kickstarter email sent to ${subscribers.length} subscribers`);
    ...
    return res.status(200).json({
      ...
      message: testEmail 
        ? `Test VIP email sent successfully to ${testEmailAddress}` 
        : `🚨 CRITICAL: VIP Kickstarter campaign sent to ${subscribers.length} subscribers! Expected 30-50 new Kickstarter followers within 24 hours.`
    });
...
🚀 FOLLOW KICKSTARTER NOW
https://www.kickstarter.com/projects/markdiorio/elekin-tcg?utm_source=email&utm_medium=vip&utm_campaign=early_access&utm_content=main_cta
