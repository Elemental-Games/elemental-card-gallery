...
const GameBoard = () => {
  ...
  const { initializeGame, startGame, playCard, endTurn } = actions;
  ...
  const handleCardClick = (cardId) => {
    ...
    if (isCardPlayable(card, playerState.essence)) {
      const zone = card.type.toLowerCase();
  ...
  return (
    <div className="relative w-full min-h-screen bg-[#1A103C] text-white">
      {/* Game Mat Background */}
      <div className="absolute inset-0 bg-center bg-cover opacity-50"
           style={{ backgroundImage: 'url(/beta/mats/Game Mat.png)' }} />
      
      {/* Game Content */}
      <div className="relative z-10 h-screen p-4">
        <div className="grid grid-rows-[auto_1fr_auto] h-full gap-4">
          {/* Game Header */}
          <div className="flex justify-between items-center p-4 bg-purple-900/60 backdrop-blur-sm rounded-lg border border-purple-500/30">
          ...
          <div className="grid grid-cols-[1fr_3fr_1fr] gap-4">
            {/* Player 1 Stats */}
            <div className="bg-purple-900/60 backdrop-blur-sm p-4 rounded-lg border border-purple-500/30">
              <h3 className="text-xl font-bold mb-4">Player 1</h3>
              <div className="space-y-2">
                <p>Health: {game.players.player1.health}</p>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(game.players.player1.essence).map(([element, amount]) => (
                    <div key={element} className="bg-purple-800/50 p-2 rounded border border-purple-500/30">
                      <img src={`/icons/${element}.png`} alt={element} className="w-6 h-6 mb-1" />
            ...
            <div className="bg-purple-900/60 backdrop-blur-sm p-4 rounded-lg border border-purple-500/30">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(game.players.player2.essence).map(([element, amount]) => (
                    <div key={element} className="bg-purple-800/50 p-2 rounded border border-purple-500/30">
                      <img src={`/icons/${element}.png`} alt={element} className="w-6 h-6 mb-1" />
                      <span className="text-yellow-400">{amount}</span>
                    </div>
