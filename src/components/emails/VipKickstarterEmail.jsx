...
const VipKickstarterEmail = () => {
  const [sampleData] = useState({
    firstName: 'Alex',
    subscriberNumber: '47',
    daysToLaunch: '14',
    kickstarterLink: 'https://www.kickstarter.com/your-project-link'
  });

  // Process the email template with sample data
  const processTemplate = (template) => {
    return template
      .replace(/\{\{FIRST_NAME\}\}/g, sampleData.firstName)
      .replace(/\{\{SUBSCRIBER_NUMBER\}\}/g, sampleData.subscriberNumber)
      .replace(/\{\{DAYS_TO_LAUNCH\}\}/g, sampleData.daysToLaunch)
      .replace(/\{\{KICKSTARTER_LINK\}\}/g, sampleData.kickstarterLink)
      .replace(/\{\{UNSUBSCRIBE_LINK\}\}/g, 'https://elementalgames.gg/unsubscribe');
  };
...
<head>
    ...
    <style>
        ...
        .benefits {
            ...
            padding: 25px;
            border-radius: 8px;
            border-left: 4px solidrgb(37, 235, 159);
            margin: 25px 0;
        }
        .benefit-item {
            display: flex;
...
<body>
  ...
  const htmlContent = processTemplate(htmlTemplate);
...
export default VipKickstarterEmail; 