...
const CardDisplay = ({ card }) => {
  ...
  return (
    <Link to={`/cards/${card.name.toLowerCase().replace(/\s+/g, '-')}`} className="block">
      <motion.div
        className="w-full aspect-[2/3] rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 relative cursor-pointer"
        ...
        >
          ...
          <div className="absolute inset-0 backface-hidden" style={{ transform: 'rotateY(180deg)' }}>
            <div className="bg-gray-800 text-white p-4 h-full overflow-y-auto">
              ...
              <p className="text-xs mb-2">{card.element} | {card.type} | {card.rarity}</p>
